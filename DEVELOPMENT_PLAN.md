...
当前进度：M1 和 M2 的首个可运行版本已完成。ENV-S3 的 MQTT 应用契约已确认并通过独立适配器接入，已完成本地 Mosquitto 的在线状态与传感器端到端验证；公网认证信息仍需通过安全渠道提供后再做真机验证。机械臂仍使用原有 Mock 契约。
...
目标：不修改 Agent Core，通过独立 DeviceGateway 适配不同固件协议。

- [x] 适配 ENV-S3 retained 在线/离线状态。
- [x] 适配温湿度、光敏、心率和血氧消息。
- [x] 适配 RGB 与灯带纯文本命令和状态回执。
- [x] 后端静态声明固件已知能力，不依赖不存在的 capabilities Topic。
- [x] 同一执行器命令串行化并等待回执。
- [x] 停止时向对应执行器发送关闭模式并等待回执。
- [ ] 使用公网认证信息完成真机联调。
- [ ] 固件回执增加 `command_id`，支持严格幂等和多实例并发。
- [ ] 验证硬件急停。
- [ ] 验证超时和断线安全状态。
- [x] 使用执行器 `state` 回执验证灯光控制结果。
- [ ] 把传感器读数接入睡眠场景判断，而不是直接使用未经确认的阈值。
...
- [ ] 公网 Broker 联调账号尚未通过安全渠道注入本地环境。
- [ ] 生产环境 TLS/WSS、设备级账号和 ACL 尚未配置。
- [ ] 固件回执缺少 `command_id`，超时重试和多实例并发无法严格关联。
- [ ] ENV-S3 没有硬件急停语义，目前只能将对应灯光切换为关闭模式。
- [ ] 传感器到睡眠场景的阈值和判断规则尚未确认。
...
- [x] 全局取消整个 Run，停止当前设备动作并跳过尚未执行的后续 Action。
